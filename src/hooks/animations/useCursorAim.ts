...
// Helpers d'orientation des faux curseurs scriptés, au scope module : fonctions pures
// (aucun état local) → pas reconstruites à chaque render, et le React Compiler peut
// mémoïser les composants consommateurs. La progression `t` (0→1) fait revenir le curseur
// à sa position naturelle sur la fin du trajet (settledAngle) : il arrive droit, juste
// avant le clic.

// Bézier quadratique : oriente la pointe le long de la courbe. Retourne la position
// calculée (utile pour coller un ghost dessus).
function bezier(t: number, p0: Pt, c: Pt, p1: Pt): Pt {
  const mt = 1 - t
...
function segment(x: number, y: number, dirX: number, dirY: number, t?: number): void {
  dispatchCursor(
    x,
    y,
...
function rest(x: number, y: number): void {
  dispatchCursor(x, y, CURSOR_REST_ANGLE)
}
