...
export function addMeasureTool(
  ...
  // Fill en premier → rendu SOUS la ligne et les points. Opacité 0 au départ : la
  // couleur n'apparaît qu'au reveal une fois la boucle fermée (sinon flash).
  if (!map.getLayer(LYR_FILL)) {
    map.addLayer({
      id: LYR_FILL,
      type: 'fill',
      source: SRC_FILL,
      paint: { 'fill-color': '#aa3bff', 'fill-opacity': 0 },
    })
  }
  if (!map.getLayer(LYR_LINE)) {
    map.addLayer({
      ...
      type: 'line',
      source: SRC_LINE,
      paint: { 'line-color': '#aa3bff', 'line-width': 3 },
    })
  }
  ...
  const refresh = () => {
    ...
    let fillFC: FeatureCollection<Polygon> = { type: 'FeatureCollection', features: [] }
    ...
    if (points.length >= 2) {
      ...
      if (closed && points.length >= 3) {
        fillFC = {
          type: 'FeatureCollection',
          features: [turf.polygon([ring]) as Feature<Polygon>],
        }
      }
    }
    setData(SRC_LINE, lineFC)
    setData(SRC_FILL, fillFC)
    onChange([...points], km)
  }
  ...
  if (interactive) {
    ...
    // le pâté sommet par sommet, on ferme la boucle, puis la couleur monte en fondu.
    ...
    if (reduced) {
      ...
      closed = true
      refresh()
      map.setPaintProperty(LYR_FILL, 'fill-opacity', FILL_OPACITY)
      ...
      const click = createTourPulse(map, '#aa3bff')
      pulse = click
      ...
      // Beat A : le curseur revient au 1er point → la boucle se referme (fill invisible).
      ...
      // Beat B : final — le curseur « presse » le coin de fermeture (onde), puis le
      // remplissage se propage DEPUIS ce coin, et la zone s'assombrit un cran.
      const REVEAL = 'reveal'
      tl.addLabel(REVEAL)
  ...
  return {
    detach() {
      ...
      for (const id of [LYR_PTS, LYR_LINE, LYR_FILL]) if (map.getLayer(id)) map.removeLayer(id)
      for (const id of [SRC_PTS, SRC_LINE, SRC_FILL]) if (map.getSource(id)) map.removeSource(id)
      points.length = 0
      onChange([], 0)
    },
  }
